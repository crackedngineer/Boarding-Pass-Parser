...
export async function signInWithGoogle(): Promise<void> {
  const data = await httpClient.post<{ url: string }>('/auth/google/signin', undefined, { includeAuth: false })
...
export async function connectGoogle(accessToken: string): Promise<void> {
  ...
  if (!res.ok) {
    ...
    throw new Error(body.error?.message ?? `connect-google failed: ${res.status}`)
  }
}
